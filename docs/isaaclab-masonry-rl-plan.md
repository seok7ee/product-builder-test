...
> 상태: **v0.4 (Atlas 자산 조사 반영)** · 대상: Isaac Lab 2.x (Isaac Sim 4.5+) · 목표: 시뮬레이션 데모까지
...
작업면  : 바닥 +211 mm → 시공 완료 시 상단 +412 mm
...
> **다만 솔직하게: 3단은 211 mm라 완전한 해방은 아닙니다.** 요구 스쿼트 깊이를 대략 절반으로 줄여줄 뿐, 1.88 m 기체에게는 여전히 상당한 굴신 구간입니다. 균형이 근본적으로 안 풀리면 3단으로도 부족할 수 있습니다. 그래서 `base_courses`를 **연속 파라미터로 구현**해 두었습니다 — 6단(0.41 m)이나 12단(0.82 m)으로 한 칸 더 올려 데모를 확정하는 선택지가 코드 수정 없이 열려 있습니다. 어느 높이에서 멈출지는 P5 결과를 보고 판단하면 됩니다.
